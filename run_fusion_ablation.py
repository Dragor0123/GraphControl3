...
class FusionAblationGraphControl(nn.Module):
    def __init__(self, base_model, variant):
        super().__init__()
        ...
        hidden_size = self.linear_classifier.in_features

        if variant == "scalar_gate":
            self.scalar_logit = nn.Parameter(torch.zeros(1))
        elif variant == "node_gate":
    ...
    def reset_classifier(self):
        ...
        if self.variant == "node_gate":
            for module in self.node_gate:
                if hasattr(module, "reset_parameters"):
                    module.reset_parameters()

    def forward_subgraph(self, x, x_sim, edge_index, batch, root_n_id, frozen=True, return_aux=False):
        if not frozen:
        ...
        h_control = self.trainable_copy.forward_subgraph(x_down, edge_index, batch, root_n_id)
        ...
        elif self.variant == "scalar_gate":
            alpha = torch.sigmoid(self.scalar_logit)
            out = alpha * h_frozen + (1.0 - alpha) * h_control
            aux["scalar_alpha"] = alpha.detach()
        elif self.variant == "node_gate":
            gate_values = torch.sigmoid(self.node_gate(torch.cat([h_frozen, h_control], dim=-1)))
            out = gate_values * h_frozen + (1.0 - gate_values) * h_control
            aux["gate_values"] = gate_values.detach()
        else:
            raise ValueError(f"Unsupported fusion variant: {self.variant}")
...
def build_parser():
    ...
    parser.description = "Run GraphControl fusion ablation experiments."
    parser.add_argument(
        "--variants",
        nargs="+",
        default=["baseline", "scalar_gate", "node_gate"],
        choices=["baseline", "scalar_gate", "node_gate"],
    )
    parser.add_argument("--output_root", type=str, default="fusion_ablation_results")
...
def build_subgraph_loaders(config, dataset_obj, train_mask, val_mask, test_mask):
    train_idx = train_mask.nonzero(as_tuple=False).view(-1)
...
def train_variant(config, model, train_loader, val_loader, device, full_x_sim):
    ...
    for epoch in progress:
        for data in train_loader:
            ...
            logits = model.forward_subgraph(x, x_sim, data.edge_index, data.batch, data.root_n_id, frozen=True)
            loss = criterion(logits, data.y)
            loss.backward()
            optimizer.step()

        if epoch % eval_steps == 0:
            val_metrics = evaluate_variant(model, val_loader, device, full_x_sim)
            progress.set_postfix({"Epoch": epoch, "ValAcc": f"{val_metrics['acc']:.4f}"})
...
@torch.no_grad()
def evaluate_variant(model, loader, device, full_x_sim):
    model.eval()
    correct = 0
    total = 0
    gate_values = []
    ...
    for batch in loader:
        ...
        logits, aux = model.forward_subgraph(
            batch.x, x_sim, batch.edge_index, batch.batch, batch.root_n_id, frozen=True, return_aux=True
        )
        preds = logits.argmax(dim=1)
        correct += (preds == batch.y).sum().item()
        total += batch.y.size(0)

        if aux["gate_values"] is not None:
            gate_values.append(aux["gate_values"].squeeze(-1).cpu())
        if aux["scalar_alpha"] is not None:
            scalar_values.append(aux["scalar_alpha"].view(1).cpu())
...
def write_variant_report(output_dir, variant, per_seed_rows, aggregate_row):
    ...
    for row in per_seed_rows:
        lines.append(
            "  Seed={Seed} ValAcc={ValAcc:.6f} TestAcc={TestAcc:.6f} BestEpoch={BestEpoch} GateMean={GateMean} GateStd={GateStd} ScalarAlpha={ScalarAlpha}".format(
                Seed=row["Seed"],
                ValAcc=row["ValAcc"],
                TestAcc=row["TestAcc"],
                BestEpoch=row["BestEpoch"],
                GateMean=format_metric(row["GateMean"]),
                GateStd=format_metric(row["GateStd"]),
                ScalarAlpha=format_metric(row["ScalarAlpha"]),
            )
        )
    lines.extend(
        [
            ...
            f"  GateMean={format_metric(aggregate_row['GateMean'])}",
            f"  GateStd={format_metric(aggregate_row['GateStd'])}",
            f"  ScalarAlpha={format_metric(aggregate_row['ScalarAlpha'])}",
        ]
    )
...
def aggregate_rows(dataset_name, variant, rows):
    ...
    return {
        ...
        "GateMean": mean_or_blank("GateMean"),
        "GateStd": mean_or_blank("GateStd"),
        "ScalarAlpha": mean_or_blank("ScalarAlpha"),
    }


def write_summary_csv(output_dir, rows):
    summary_path = output_dir / "summary.csv"
    fieldnames = ["Dataset", "Variant", "ValAcc", "TestAcc", "BestEpoch", "GateMean", "GateStd", "ScalarAlpha"]
...
def main(config):
    ...
    for variant in config.variants:
        per_seed_rows = []
        print(f"Running fusion variant: {variant}")
        for seed_idx, seed in enumerate(config.seeds):
            ...
            train_loader, val_loader, test_loader = build_subgraph_loaders(
                config, dataset_obj, train_mask, val_mask, test_mask
            )

            base_model = load_model(num_node_features, dataset_obj.num_classes, config).to(device)
            model = FusionAblationGraphControl(base_model, variant).to(device)

            best_epoch, best_val_acc = train_variant(config, model, train_loader, val_loader, device, full_x_sim)
            test_metrics = evaluate_variant(model, test_loader, device, full_x_sim)

            row = {
                "Seed": seed,
                "ValAcc": best_val_acc,
                "TestAcc": test_metrics["acc"],
                "BestEpoch": best_epoch,
                "GateMean": test_metrics["gate_mean"],
                "GateStd": test_metrics["gate_std"],
                "ScalarAlpha": test_metrics["scalar_alpha"],
            }
            per_seed_rows.append(row)
            print(
                f"Seed: {seed}, Variant: {variant}, ValAcc: {best_val_acc:.4f}, "
                f"TestAcc: {test_metrics['acc']:.4f}, BestEpoch: {best_epoch}"
            )
    ...
    print(f"Saved fusion ablation results to {output_dir}")
